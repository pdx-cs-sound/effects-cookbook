...
- **Real time first.** Effects are chosen to be achievable in real time at reasonable
  cost, and effects that need the whole signal before producing any output are out of
  scope. The reference code is offline Python, one whole signal in a list, because that
  is the clearest way to read an algorithm; every algorithm is still a single pass that
  could run as samples arrive. Where real time imposes a cost of its own (latency,
  lookahead, memory), the effect's pitfalls say so.
...
The code on the effect pages is included at build time from tested modules in the
repository's `code/` directory, and the figures and audio demos are generated by
committed scripts that run the same modules. Page, figure, and sound come from one
