...
AGC is less common as a named audio effect than compression or limiting, but the
function has not disappeared. In digital systems it shows up differently depending on
whether the entire signal is available for analysis.
...
- Telephony and VoIP. ITU-T G.169 specifies automatic level control (ALC) for digital
  network equipment. WebRTC's AGC2 is an open-source implementation of digital automatic
  gain control.
...
level-following machinery rather than as a completely separate class of effect.
